...
functions.cloudEvent('helloGCS', async (cloudEvent) => {
  ...
  const destinationFile = `/tmp/converted/${fileNameWithoutExtension}-converted.pdf`; // Append "-resized" to the filename

  await bucket.file(file.name).download({
    destination: originalFile
  });

  const pngBuffer = fs.readFileSync(originalFile);

  const png = PNG.sync.read(pngBuffer);

  const pdf = new PDFDocument({ autoFirstPage: false });

  pdf.addPage({ size: [png.width, png.height] });
